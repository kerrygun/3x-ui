...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        ...
        wget --inet4-only -N -O /usr/local/x-ui-linux-$(arch).tar.gz https://github.com/${GITHUB_REPO}/releases/download/${tag_version}/x-ui-linux-$(arch).tar.gz
        if [[ $? -ne 0 ]]; then
    ...
    wget --inet4-only -O /usr/bin/x-ui-temp https://raw.githubusercontent.com/${GITHUB_REPO}/main/x-ui.sh
    if [[ $? -ne 0 ]]; then
    ...
    if [[ $release == "alpine" ]]; then
        wget --inet4-only -O /etc/init.d/x-ui https://raw.githubusercontent.com/${GITHUB_REPO}/main/x-ui.rc
        if [[ $? -ne 0 ]]; then
